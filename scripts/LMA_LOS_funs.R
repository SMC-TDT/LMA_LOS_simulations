...
area_by_num <- function(rast){
  
  # Count number of occurrences and calculate area
  freq_df <- as.data.frame(freq(rast))
  colnames(freq_df) <- c("num_lma", "num_pix")
  freq_df <- freq_df[order(freq_df$num_lma, decreasing = T),]
  freq_df$area <- prod(res(rast))*freq_df$num_pix/1000000 #in km^2
  freq_df <- subset(freq_df, selec=c("num_lma", "area"))
  freq_df$area_accum <- cumsum(freq_df$area)
  freq_df$area <- sprintf("%.2f", freq_df$area)
  freq_df$area_accum <- sprintf("%.2f", freq_df$area_accum)
  
  freq_df <- freq_df[complete.cases(freq_df$num_lma), ]
...
plot_hmin <- function(file_ou, rast, rast_dem, shpfile, point_utm, 
                      ...
                      xlims, ylims, pl_tit, leg_tit, 
                      sea_col='white', shp_over=F){
...
write_log <- function(data_df, f_out, lines){
  
  cat(lines, file=f_out, append=F)
  write.table(data_df, file = f_out, append = T, 
